...
-- ESP defaults (overridden by ESP_CONFIG on ESP.start)
	...
	RepSyncQueueScanSec = 2.5,       -- интервал pairs(rep) для новых UID
...
local function refreshActorClientsFromGC()
	if type(getgc) ~= "function" then return end
	local now = os.clock()
	-- v18: GC скан очень дорог — не чаще 1 раза в 30 секунд
...
local function considerReplicatorActor(uid, actorData, found)
	...
	if isNpcActor then
		...
		if ownerName and ownerName ~= "" then npcLabel = ownerName end
...
local function finalizeActorScan(found)
	...
	if prevCount < maxTracked then
		for uid, data in pairs(prev) do
			if not found[uid] and data.model and data.model.Parent
				and not Bridge.isActorDead(data) then
...
local function scanActors()
	...
	local found = {}
	-- FIX v20: полный O(N) проход по Replicator.Actors убран — NPC подтягиваются батчами.
	syncReplicatorActorsTable()
...
local function tickRepSyncBatch(batchSize)
	...
	-- FIX v11: Player Priority Queue — игроки обновляются НЕМЕДЛЕННО, без ожидания своей очереди
...
-- Единая точка прицеливания: v138, silent aim и aim-viz используют одно и то же.
...
function Bridge.shouldSkipActorCollect(class, player, squad, teamKey, uid)
	...
	if class == "self" or class == "dead" then
		return true
	end
...
-- Строгий LOS для ESP: пробиваемые препятствия считаются блоком (игра пробивает только Glass)
...
function Bridge.resolveResolverLite(muzzleOrigin, model, uid, cam, maxAngle)
	...
	if rlHead and typeof(muzzleOrigin) == "Vector3" then
		local rlDist = (rlHead.Position - muzzleOrigin).Magnitude
...
function Bridge.collectAimActorCandidates(cam, losOrigin, maxDist, maxAngle)
	...
	if not enemyCache or (now - (State._aimEnemyCacheT or 0)) > 0.15
		...
		for _, data in pairs(State.actors) do
			if Bridge.isEnemyActor(data) then
				if not (CONFIG.SilentAimIgnoreNpc == true and Bridge.isNpcActorClass(data.class)) then
...
function Bridge.patchV138ServerAim(v138)
	...
	-- 3. Получить актуальную цель (не использовать кеш — нам нужна позиция СЕЙЧАС)
	local target = State.shotAimTarget
...
function Bridge.predictAimPoint(uid, currentPos, origin, bulletSpeed, part, _extraTime)
	...
	local tFlight = predictionFlightTime(muzzleOrigin, currentPos, bulletSpeed)

	-- FIX: PingCompensation убрана из предикта — сервер игры сам компенсирует RTT.
	-- Добавление pingMs сверху давало двойную компенсацию и overshooting.
	-- tFlight уже точно отражает время полёта пули с учётом баллистики.

	local velMag = vel.Magnitude
	local velCap = CONFIG.PredictionMaxVelCap or 35  -- studs/s, ~WalkSpeed*2
	local velScaled = velMag > velCap and vel * (velCap / velMag) or vel

	local tTotal = tFlight
	local predicted = currentPos + velScaled * tTotal
