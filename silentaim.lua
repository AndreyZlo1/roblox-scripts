...
local SA_CONFIG = {
	...
	SwastikaRGB = true,
	ForceHitTimeOff = 0,
	-- Backtrack удалён v4 (не работал)
...
local HIT_FX_FOLDER_NAME = "BRM5_HitFX"
State.lastHitFxAt = State.lastHitFxAt or 0

local function getHitFxFolder()
	local ws = workspace
	local f = ws:FindFirstChild(HIT_FX_FOLDER_NAME)
	if not f then
		f = Instance.new("Folder")
		f.Name = HIT_FX_FOLDER_NAME
		f.Parent = ws
	end
	return f
end
...
function Bridge.hookDischargeClosure()
	-- Отключено: дублирует namecall FireServer patch.
	return false
end
...
function Bridge.updateAimVisuals()
	...
	local function ensureCircle(key, numSides, zIndex)
		if not viz[key] then
			viz[key] = Drawing.new("Circle")
			viz[key].Filled = false
			viz[key].NumSides = numSides or 12
			viz[key].ZIndex = zIndex or 46
		end
		return viz[key]
	end

	local function drawWorldCircle(circle, worldPos, radius, color, alpha)
		...
		circle.Color = color
		Bridge.showDrawing(circle, alpha)
		return true
	end
...
local function resetAfterRespawn()
	...
	-- v18 PATCH: сброс locked resolvers — без этого resolveEquippedHand использует метод от старого клиента
	State.methods = {}
